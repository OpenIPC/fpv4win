...
bool GifEncoder::open(int width, int height, AVPixelFormat pixelFormat, int frameRate, const string& outputPath) {
    ...
    //设置编码参数
    _codecCtx = shared_ptr<AVCodecContext>(avcodec_alloc_context3(pCodec),[](AVCodecContext * ctx){
    ...
    if(_codecCtx->pix_fmt!=pixelFormat){
        //颜色转换器
        _imgConvertCtx = sws_getCachedContext(_imgConvertCtx, width, height, pixelFormat,
                                              _codecCtx->width, _codecCtx->height, _codecCtx->pix_fmt, SWS_BICUBIC, nullptr, nullptr, nullptr);
        if(!_imgConvertCtx){
            return false;
        }
    }
...
bool GifEncoder::encodeFrame(const shared_ptr<AVFrame> &frame) {
    ...
    if(_codecCtx->pix_fmt!=frame->format){
        //分配帧空间
        if(!_tmpFrame){
            _tmpFrame = shared_ptr<AVFrame>(av_frame_alloc(),[](AVFrame * f){
                av_frame_free(&f);
            });
            if (!_tmpFrame){
                return false;
            ...
            _buff.resize(size);
            int ret = av_image_fill_arrays(_tmpFrame->data,_tmpFrame->linesize, _buff.data(),_codecCtx->pix_fmt, _codecCtx->width, _codecCtx->width,1);
            if(ret<0){
                return false;
            }
        }
        //转换为GIF编码需要的颜色和高度
        int h = sws_scale(_imgConvertCtx, frame->data, frame->linesize, 0, frame->height, _tmpFrame->data, _tmpFrame->linesize);
    ...
    //记录帧编码时间
    _lastEncodeTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    //发送帧到编码上下文
