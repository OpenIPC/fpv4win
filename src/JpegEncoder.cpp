...
bool JpegEncoder::encodeJpeg(const string &outFilePath, const shared_ptr<AVFrame> &frame) {
    ...
    //编码上下文
    shared_ptr<AVFormatContext> pFormatCtx = shared_ptr<AVFormatContext>(avformat_alloc_context(),&avformat_free_context);
    ...
    //设置编码参数
    shared_ptr<AVCodecContext> codecCtx = shared_ptr<AVCodecContext>(avcodec_alloc_context3(pCodec),[](AVCodecContext * ctx){
