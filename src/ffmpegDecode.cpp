...
//    pFormatCtx->probesize = 0;
    ...
    //创建音频解码缓存
    if(hasAudioStream){
        audioFifoBuffer = shared_ptr<AVFifoBuffer>(av_fifo_alloc(GetAudioFrameSamples()*GetAudioChannelCount()*10),
                                                   &av_fifo_free);
    }
	return true;
}
    ...
    while (packetSize > 0){
        ...
        if (packetDecodedSize >= 0) {
            if (audioFrame->format != AV_SAMPLE_FMT_S16) {
                ...
                if (!swrCtx) {
                	auto ptr = swr_alloc_set_opts(nullptr, pAudioCodecCtx->channel_layout, AV_SAMPLE_FMT_S16, pAudioCodecCtx->sample_rate,
																	 pAudioCodecCtx->channel_layout,
																	 static_cast<AVSampleFormat>(audioFrame->format), pAudioCodecCtx->sample_rate, 0, nullptr);
